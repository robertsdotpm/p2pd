...
class TestWarpgateServer(AsyncTestCase):
    async def test_warpgate_server(self):
        return  # TODO: fix this test

        # Start the Warpgate server.
        af = IP4
        i = await Interface().start()
        nic_ip = i.route(af).nic()
        r = await i.route(af).bind(ips=nic_ip, port=WARPGATE_PORT)
        server = await start_warpgate_server(r, ifs=[i], enable_upnp=False)
